...
- Explicit Suspension Topologies: Geometry files select a concrete type rather than activating features through optional hardpoints: `double_wishbone`, `double_wishbone_coilover`, `double_wishbone_pushrod_rocker`, `double_wishbone_pushrod_rocker_arb` (a standalone corner with an ARB pickup), `double_wishbone_axle`, or `double_wishbone_pushrod_rocker_axle`.
- Clear Model Hierarchy: Single-corner models live under `kinematics.suspensions.corner`; axle models live under `kinematics.suspensions.axle` and extend corners with pairing and connection logic.
...
- Inboard Actuation: The `double_wishbone_pushrod_rocker` topology explicitly selects a torsion bar or inboard coilover spring. The axle form adds an inboard anti-roll bar actuated through rocker droplinks.
...
- Spring/Damper Element: `double_wishbone_coilover` mounts a coilover to the lower wishbone; a pushrod-rocker geometry can explicitly select an inboard coilover instead. Both provide damper length and motion-ratio outputs.
