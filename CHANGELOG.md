...
### Added
- Explicit suspension topology catalogue: coilover, pushrod-rocker, basic axle, and pushrod-rocker/ARB axle geometries now use distinct type keys and concrete classes. Corner models live under `suspensions/corner/`; axle models live under `suspensions/axle/`, where they compose and extend corner models with connection logic. Pushrod-rocker schemas require an explicit `spring.type` (`torsion_bar` or `coilover`); variant behavior is no longer inferred from whichever optional hardpoints happen to be present. Parsing, construction, supported-type discovery, and runtime class lookup all use one suspension definition registry.
...
- New `kinematics.schema` package holding the transport-agnostic, validated input models and coercion helpers (no file IO):
  ...
  - `schema/geometry.py` -- one strict specification model per concrete suspension type, selected through the suspension definition catalogue by `parse_geometry_spec()`. The axle hardpoints block is a real model (`AxleHardpointsSpec`) that validates mirror vs explicit mode.
...
- Explicit coilover topologies using `strut_top`/`strut_bottom`: `double_wishbone_coilover` mounts the damper to the lower wishbone, while a pushrod-rocker geometry with `spring.type: coilover` mounts it to the rocker. Both export `damper_length_mm` and drive `damper_motion_ratio`.
...
- F1-style inboard actuation for the explicit pushrod-rocker corner and axle types: pushrod to an inboard rocker rotating about a chassis-fixed axis (validated parallel to the XZ plane), an explicitly selected torsion bar or coilover spring, and an inboard anti-roll bar on the axle type.
  ...
  - New metrics: `rocker_angle_deg` per rocker corner, `torsion_bar_twist_deg` only when `spring.type: torsion_bar`, per-corner `arb_arm_angle_deg` on the ARB axle, and axle-level `arb_twist_deg = left - right`.
...
- Torsion-bar columns (`torsion_bar_twist_deg`, `torsion_bar_motion_ratio_deg_per_mm`) are emitted only for pushrod-rocker types that explicitly select `spring.type: torsion_bar`; their values remain coaxial aliases of the rocker columns.
