...
The coilover top is chassis-fixed and its bottom pickup is rigidly attached to
the lower wishbone. Its length is deliberately unconstrained so it can change
through wheel travel.
"""
...
class DoubleWishboneCoiloverSuspension(DoubleWishboneSuspension):
    ...
    def output_points(self) -> tuple[PointID, ...]:
        """Base outputs plus the two coilover points."""
        return (*super().output_points(), PointID.STRUT_TOP, PointID.STRUT_BOTTOM)
